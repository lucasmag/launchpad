...
export function useFetchSong() {
  const [state, setState] = React.useState<SongLoadState>(
  ...
  async function fetchSongZip(songName: string): Promise<JSZip> {
    ...
    const response = await fetch(`${SERVER_URL}/songs/${songName}`).catch(
      (error) => {
  ...
  function extractSongSetAndSoundName(filename: string) {
    ...
    return {
      songSet: songSet ? (parseInt(songSet) as SongSet) : null,
      soundName,
    };
  }

  function constructAudioTrackMapping(
    songName: string,
    fileByName: Record<string, Blob>,
  ) {
    ...
    Object.entries(localSongs[songName]).forEach(
      ([songSet, soundURLMapping]) => {
        Object.entries(soundURLMapping).forEach(([soundName, fileURL]) => {
          audioTrackMapping[parseInt(songSet) as SongSet][soundName] = new Howl(
  ...
  async function fetchSong(songName: string) {
    const audioTrackMapping: Record<
      SongSet,
      Record<string, Howl>
    > = loadAudioTrackMappingFromState(songName) ||
    (await loadAudioTrackMappingFromServer(songName));
